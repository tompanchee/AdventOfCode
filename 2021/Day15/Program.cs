...
namespace Day15
{
    class Program
    {
        static void Main(string[] args) {
            ...
            var (risk, _) = graph.FindShortestPath((0, 0), (data.Length - 1, data[0].Length - 1), Euclidean);
            ...
            (risk, _) = graph.FindShortestPath((0, 0), (data.Length * 5 - 1, data[0].Length * 5 - 1), Euclidean);
            Console.WriteLine($"Minimum total risk through cave is {risk} in {sw.Elapsed}");
        }
        ...
        private static Graph<(int row, int col)> BuildGraph(string[] data) {
        ...
        private static Graph<(int row, int col)> BuildGraph5x(string[] data) {
            (int dr, int dc)[] offsets = {
                (-1, 0),
                (0, 1),
                (1, 0),
            ...
            return new Graph<(int row, int col)>(vertices, 2500);
        }
    }
}